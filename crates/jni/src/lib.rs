//! JNI entry points for `com.fulcrumgenomics.fgkl.pairhmm.FgklPairHmm` and
...
fn compute(
    ...
    let bases = env.convert_byte_array(bases)?;
    let quals = env.convert_byte_array(quals)?;
    let ins_gop = env.convert_byte_array(ins_gop)?;
    let del_gop = env.convert_byte_array(del_gop)?;
    let gcp = env.convert_byte_array(gcp)?;
    let read_offsets = int_array(env, read_offsets)?;
    let hap_bases = env.convert_byte_array(hap_bases)?;
    let hap_offsets = int_array(env, hap_offsets)?;

    let reads = split(&read_offsets, bases.len())?
        .into_iter()
        .map(|(lo, hi)| ReadRef {
            bases: &bases[lo..hi],
            quals: &quals[lo..hi],
            ins_gop: &ins_gop[lo..hi],
            del_gop: &del_gop[lo..hi],
            gcp: &gcp[lo..hi],
        })
        .collect::<Vec<_>>();
    if [quals.len(), ins_gop.len(), del_gop.len(), gcp.len()].iter().any(|&l| l != bases.len()) {
        return Err(Failure {
            class: ILLEGAL_ARGUMENT,
